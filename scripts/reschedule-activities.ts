...
import { sendSessionCancellationEmail } from '../src/lib/email';

/**
 * One-off schedule change: replaces every current/future session for the five weekday
 * activities and School Tour with a fixed 10-week term (1:30pm-3:30pm for the weekday
 * activities, hourly 9am-1pm Mon-Fri for School Tour). Any existing session for these
 ...
 * "Cancel session" button sends) if it has real bookings. Past sessions are left untouched,
 * and no other activity (e.g. Adventure Camp) is touched.
 *
...
async function main() {
  ...
  console.log(`Created ${created} new session(s) for the ${TERM_WEEKS}-week term starting ${toDateString(TERM_START)}.`);
  ...
  if (staleIds.length === 0) {
    ...
    const result = await removeStaleSessions(staleIds);
    console.log(
