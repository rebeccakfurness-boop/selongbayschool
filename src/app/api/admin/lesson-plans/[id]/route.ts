...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await ensureSchema();
    const rows = await sql`SELECT class_name FROM lesson_plans WHERE id = ${id}`;
    if (rows.length === 0) {
    ...
    await sql`DELETE FROM lesson_plans WHERE id = ${id}`;
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[api/admin/lesson-plans/:id] failed to delete', err);
    return NextResponse.json({ error: 'Could not delete lesson plan.' }, { status: 500 });
  }
}
