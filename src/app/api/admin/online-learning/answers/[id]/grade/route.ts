...
import { getClassNameForAnswerSubmission, gradeLessonAnswer } from '@/lib/online-learning';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await ensureSchema();
    const className = await getClassNameForAnswerSubmission(submissionId);
    if (!className) return NextResponse.json({ error: 'Answer not found.' }, { status: 404 });
    if (!(await canAccessClass(staff, className))) {
      return NextResponse.json({ error: 'You are not assigned to that class.' }, { status: 403 });
    }

    await gradeLessonAnswer(submissionId, parsed.data.grade, parsed.data.comment ?? null, staff.adminUserId);
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[api/admin/online-learning/answers/:id/grade] failed', err);
