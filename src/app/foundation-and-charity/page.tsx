...
import DisasterResponseFunds from '@/components/DisasterResponseFunds';
import { charitableWork, communityPartners, disasterResponseSection } from '@/lib/site-content';
...
export default function FoundationAndCharityPage() {
  return (
    <div className="flex flex-col gap-16 pb-20 md:gap-24">
      ...
      <Reveal>
        <div className="mx-auto max-w-4xl px-6 md:px-8">
          <p className="font-script text-3xl text-orange-deep md:text-4xl">{disasterResponseSection.eyebrow}</p>
          <h2 className="mt-1 font-display text-3xl font-semibold text-ink md:text-4xl">{disasterResponseSection.heading}</h2>
          <p className="mt-3 max-w-prose text-[15px] leading-relaxed text-ink-soft">{disasterResponseSection.intro}</p>
          <DisasterResponseFunds />
        </div>
      </Reveal>
