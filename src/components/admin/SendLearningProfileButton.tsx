...
/** Mirrors SendInvoiceButton exactly — manual-only email send, admin-only, only meaningful once
 * the report is approved (the server rejects it otherwise). */
export default function SendLearningProfileButton({
  profileId,
  defaultEmail,
  initiallySent,
}: {
  ...
  async function send() {
    ...
    try {
      const res = await fetch(`/api/admin/learning-profiles/${profileId}/send`, {
        method: 'POST',
  ...
  if (!expanded) {
    return (
      <button type="button" onClick={() => setExpanded(true)} className="text-xs font-semibold text-teal-deep hover:underline">
        {sent ? 'Sent ✓ (send again)' : 'Send to parent'}
      </button>
    );
  }
