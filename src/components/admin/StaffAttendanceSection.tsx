'use client';

import { useEffect, useState } from 'react';
import { formatDateTime } from '@/lib/admin-format';

interface StaffAttendanceHistoryRow {
  id: number;
  event_type: 'check_in' | 'check_out';
  occurred_at: string;
  source: 'self' | 'admin';
  performed_by_label: string | null;
}

function sourceLabel(row: StaffAttendanceHistoryRow): string {
  if (row.source === 'self') return 'Self check-in/out';
  return row.performed_by_label ? `Admin override: ${row.performed_by_label}` : 'Admin override';
}

/** YYYY-MM-DDTHH:mm in the browser's own local time, same helper as the Child Card's
 * AttendanceSection. */
function nowForDatetimeLocalInput(): string {
  const d = new Date();
  const pad = (n: number) => String(n).padStart(2, '0');
  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
}

/** Self-fetching, same reason as the other Staff Card sections -- this also renders on a
 * teacher's own read-only view of their Staff Card (self-service "when did I actually check
 * in/out"), not just the admin one. Only an admin (canEdit) gets the correction form and the
 * delete action; a self-viewer sees history only, same as a parent can't edit their own child's
 * attendance history directly either. */
export default function StaffAttendanceSection({ adminUserId, canEdit }: { adminUserId: number; canEdit: boolean }) {
  const [history, setHistory] = useState<StaffAttendanceHistoryRow[] | null>(null);
  const [openDays, setOpenDays] = useState<string[]>([]);
  const [showCorrection, setShowCorrection] = useState(false);
  const [eventType, setEventType] = useState<'check_in' | 'check_out'>('check_in');
  const [occurredAt, setOccurredAt] = useState('');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch(`/api/admin/staff/${adminUserId}/attendance`)
      .then((res) => res.json().then((data) => ({ ok: res.ok, data })))
      .then(({ ok, data }) => {
        if (cancelled || !ok) return;
        setHistory(data.history);
        setOpenDays(data.openDays ?? []);
      });
    return () => {
      cancelled = true;
    };
  }, [adminUserId]);

  async function load() {
    const res = await fetch(`/api/admin/staff/${adminUserId}/attendance`);
    const data = await res.json().catch(() => ({}));
    if (res.ok) {
      setHistory(data.history);
      setOpenDays(data.openDays ?? []);
    }
  }

  async function submitCorrection() {
    if (!occurredAt) {
      setError('Pick a date and time.');
      return;
    }
    setSaving(true);
    setError(null);
    try {
      const res = await fetch(`/api/admin/staff/${adminUserId}/attendance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventType, occurredAt: new Date(occurredAt).toISOString() }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || 'Could not save the correction.');
      setOccurredAt('');
      setShowCorrection(false);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not save the correction.');
    } finally {
      setSaving(false);
    }
  }

  async function deleteEntry(eventId: number) {
    setHistory((prev) => (prev ? prev.filter((h) => h.id !== eventId) : prev));
    await fetch(`/api/admin/staff/${adminUserId}/attendance/${eventId}`, { method: 'DELETE' });
    load();
  }

  return (
    <div className="rounded-md border border-sand-line bg-paper p-6 shadow-soft">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-lg font-semibold text-ink">Attendance</h2>
        {canEdit && (
          <button
            type="button"
            onClick={() => {
              setShowCorrection((s) => !s);
              if (!showCorrection && !occurredAt) setOccurredAt(nowForDatetimeLocalInput());
            }}
            className="text-xs font-semibold text-teal-deep hover:underline"
          >
            {showCorrection ? 'Cancel' : '+ Check in/out (admin override)'}
          </button>
        )}
      </div>

      {openDays.length > 0 && (
        <p className="mt-2 rounded-sm bg-orange/10 px-3 py-2 text-xs font-semibold text-orange-deep">
          Checked in with no check-out on {openDays.length} day{openDays.length === 1 ? '' : 's'}: {openDays.join(', ')}
        </p>
      )}

      {canEdit && showCorrection && (
        <div className="mt-3 border-b border-sand-line pb-4">
          <p className="mb-2 text-xs text-ink-soft">
            Records a check-in/out on this staff member&apos;s behalf (e.g. they forgot to clock out). Every
            self check-in/out is recorded straight from the staff member&apos;s own logged-in session; this is the
            explicit office override for a missed one.
          </p>
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-ink-soft" htmlFor={`sa-correction-type-${adminUserId}`}>Type</label>
              <select
                id={`sa-correction-type-${adminUserId}`}
                value={eventType}
                onChange={(e) => setEventType(e.target.value as 'check_in' | 'check_out')}
                className="rounded-sm border border-sand-line bg-white px-2 py-1.5 text-sm text-ink"
              >
                <option value="check_in">Check In</option>
                <option value="check_out">Check Out</option>
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-ink-soft" htmlFor={`sa-correction-time-${adminUserId}`}>Date &amp; time</label>
              <input
                id={`sa-correction-time-${adminUserId}`}
                type="datetime-local"
                value={occurredAt}
                onChange={(e) => setOccurredAt(e.target.value)}
                className="rounded-sm border border-sand-line bg-white px-2 py-1.5 text-sm text-ink"
              />
            </div>
            <button
              type="button"
              onClick={submitCorrection}
              disabled={saving}
              className="rounded-full bg-teal px-4 py-1.5 text-xs font-bold text-white hover:bg-teal-deep disabled:opacity-60"
            >
              {saving ? 'Saving…' : 'Save'}
            </button>
            {error && <span className="text-xs font-semibold text-orange-deep">{error}</span>}
          </div>
        </div>
      )}

      <ul className="mt-3 flex flex-col gap-1.5">
        {history?.slice(0, 20).map((row) => (
          <li key={row.id} className="flex items-center justify-between gap-2 text-sm">
            <span className="text-ink">{row.event_type === 'check_in' ? 'Checked in' : 'Checked out'}</span>
            <span className="flex items-center gap-2 text-xs text-ink-soft">
              {formatDateTime(row.occurred_at)} · {sourceLabel(row)}
              {canEdit && (
                <button type="button" onClick={() => deleteEntry(row.id)} className="font-semibold text-orange-deep hover:underline">
                  Delete
                </button>
              )}
            </span>
          </li>
        ))}
        {history?.length === 0 && <li className="text-sm text-ink-soft">No attendance recorded yet.</li>}
        {history === null && <li className="text-sm text-ink-soft">Loading…</li>}
      </ul>
    </div>
  );
}
