...
export function sortSchedule(rows: ClassScheduleRow[]): ClassScheduleRow[] {
...
/** Used by "My Roster" — a teacher's own weekly classes, merged client-side with their duty_roster
 * rows (see duty-roster.ts) into a single day-by-day view. */
export async function getWeeklyScheduleForTeacher(teacherId: number): Promise<ClassScheduleRow[]> {
  const rows = (await sql`
    SELECT cs.id, cs.class_name, cs.subject, cs.teacher_id,
      COALESCE(au.display_name, au.email) AS teacher_label,
      cs.day_of_week, cs.start_time::text, cs.end_time::text, cs.format, cs.location_or_link,
