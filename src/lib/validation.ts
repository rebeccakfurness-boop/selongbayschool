...
/** A question the student answers by typing or recording their own words instead of picking an
...
export const createQuizQuestionSchema = z.union([multipleChoiceQuestionSchema, openResponseQuestionSchema]);
...
/** A student's answer to an open_response question -- exactly one of answerText/answerAudioUrl,
 * matching curriculum_lesson_answer_submissions' own CHECK constraint. audioUrl is a Vercel Blob
 * URL from a prior upload (see /api/student/curriculum/answers/upload), never a raw file here. */
export const submitAnswerSchema = z
  .object({
    quizQuestionId: z.coerce.number().int().positive(),
    answerText: z.string().trim().max(4000).nullable().optional(),
    answerAudioUrl: z.string().trim().url().nullable().optional(),
  })
  .refine((d) => !!d.answerText || !!d.answerAudioUrl, { message: 'Type an answer or record one.' });
export type SubmitAnswerInput = z.infer<typeof submitAnswerSchema>;

export const gradeAnswerSchema = z.object({
  grade: z.string().trim().min(1).max(50).nullable(),
...
export type GradeLessonWorksheetInput = z.infer<typeof gradeLessonWorksheetSchema>;
